...
{
    if (node == NULL) {
        HILOGE("GetValue failed, node is null");
        return false;
    }
    xmlChar *nodeContent = xmlNodeGetContent(node->children);
    if (nodeContent == NULL) {
        HILOGE("GetValue failed, node content is null");
        return false;
    }
    ...
    if (!OHOS::bluetooth::ConvertStrToDigit(sValue, value, HEX_STRING_TO_INT)) {
        HILOGE("GetValue failed, convert string to digit failed");
        xmlFree(nodeContent);
        return false;
    }
    xmlFree(nodeContent);
...
bool XmlParse::Parse()
{
    ...
    if (pimpl->doc_ == NULL) {
        HILOGE("%{public}s Init failed, read file failed.", this->filePath_.c_str());
        return false;
    }
    return true;
}
...
bool XmlParse::Save()
{
    ...
    if (result == -1) {
        HILOGE("%{public}s save failed.", this->filePath_.c_str());
        return false;
    }
    return true;
}
