    ...
    for (auto it = owner_->observers_.begin(); it != owner_->observers_.end(); ++it) {
        if (it->first != nullptr && it->first->AsObject() == object) {
            if (!it->first->AsObject()->RemoveDeathRecipient(it->second)) {
                HILOGD("Failed to unlink death recipient from observer");
            }
            owner_->observers_.erase(it);
            break;
        }
    }
    HILOGD("Callback from dead process unregistered");
}

template <typename T>
bool RemoteObserverList<T>::UnregisterInternal(typename ObserverMap::iterator iter)
{
    HILOGD("RemoteObserverList<T>::UnregisterInternal called");
...
template <typename T>
void RemoteObserverList<T>::Clear(void)
{
    std::lock_guard<std::mutex> lock(lock_);
    for (const auto &[observer, deathRecipient] : observers_) {
        if (observer && observer->AsObject()) {
            observer->AsObject()->RemoveDeathRecipient(deathRecipient);
        }
    }
    observers_.clear();
}
