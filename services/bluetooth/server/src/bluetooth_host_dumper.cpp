 ...
 */
#ifndef LOG_TAG
#define LOG_TAG "bt_server_host_dumper"
#endif
...
void BluetoothHostDumper::BluetoothDump(const std::vector<std::string>& args, std::string& result)
{
    ...
    if (args.size() == MIN_ARGS_SIZE) {
        ...
        // -shwoRssroxyMap
        if (args[0] == ARGS_SHOW_RSSPROXY_MAP) {
            ShowRssFreezeInfo(result);
            return;
        }
    }
...
void BluetoothHostDumper::BtCommStateDump(std::string& result)
{
    ...
    int32_t state;
    hostServer->GetBtState(state);
    // bt conn state
    std::string btState = (state == static_cast<int>(BluetoothState::STATE_ON)) ? "enabled\n" : "disabled\n";
    result.append("Bt enable state: ").append(btState);

    // ble conn state
    bool isBleEnable = (state == static_cast<int>(BluetoothState::STATE_ON) ||
        state == static_cast<int>(BluetoothState::STATE_BLE_ON) ||
        state == static_cast<int>(BluetoothState::STATE_TURNING_ON) ||
        state == static_cast<int>(BluetoothState::STATE_TURNING_OFF));
    std::string bleState = isBleEnable ? "enabled\n" : "disabled\n";
    result.append("Ble enable state: ").append(bleState);
}

void BluetoothHostDumper::ShowRssFreezeInfo(std::string& result)
{
    sptr<BluetoothHostServer> hostServer = BluetoothHostServer::GetInstance();
    std::string res = hostServer->DumpRssFreezeInfo();
    result.append(res);
}
