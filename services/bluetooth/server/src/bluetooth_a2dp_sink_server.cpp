 ...
 */
#ifndef LOG_TAG
#define LOG_TAG "bt_server_a2dp_sink"
#endif
...
struct BluetoothA2dpSinkServer::impl {
    ...
    std::unique_ptr<A2dpSinkObserver> observerImp_ = nullptr;
    std::shared_ptr<BluetoothObserverApplicationContainer> appContainer_ = nullptr;
    IProfileA2dpSnk *a2dpSnkService_ = nullptr;
};

class BluetoothA2dpSinkServer::impl::A2dpSinkObserver : public IA2dpObserver {
public:
    explicit A2dpSinkObserver(BluetoothA2dpSinkServer::impl *pimpl) : pimpl_(pimpl) {};
    ...
    void OnConnectionStateChanged(const RawAddress &device, int state, int cause) override
    {
        ...
        observers_->ForEach([this, device, state, cause](sptr<IBluetoothA2dpSinkObserver> observer) {
...
public:
    explicit SystemStateObserver(BluetoothA2dpSinkServer::impl *pimpl) : pimpl_(pimpl) {};
            ...
            case BTSystemState::ON:
                if (serviceMgr != nullptr) {
                    ...
                    if (pimpl_->a2dpSnkService_ != nullptr) {
                        pimpl_->a2dpSnkService_->RegisterObserver(
...
BluetoothA2dpSinkServer::BluetoothA2dpSinkServer()
{
    ...
    pimpl->observerImp_ = std::make_unique<impl::A2dpSinkObserver>(pimpl.get());
...
void BluetoothA2dpSinkServer::DeregisterObserver(const sptr<IBluetoothA2dpSinkObserver> &observer)
{
    ...
    if (pimpl != nullptr) {
        ...
        pimpl->observersTokens_.Erase(observer->AsObject());
    }
}
...
int BluetoothA2dpSinkServer::Connect(const RawAddress &device)
{
    ...
    return pimpl->a2dpSnkService_->Connect(realAddr);
}
...
int BluetoothA2dpSinkServer::Disconnect(const RawAddress &device)
{
    ...
    return pimpl->a2dpSnkService_->Disconnect(realAddr);
}
...
int BluetoothA2dpSinkServer::GetDeviceState(const RawAddress &device)
{
    ...
    return pimpl->a2dpSnkService_->GetDeviceState(realAddr);
}
...
std::vector<RawAddress> BluetoothA2dpSinkServer::GetDevicesByStates(const std::vector<int32_t> &states)
{
    ...
    std::vector<RawAddress> randomRawDevices;
    BluetoothDeviceManager::GetInstance()->ConvertToRandomAddress(rawDevices, randomRawDevices);
    return randomRawDevices;
}
...
int BluetoothA2dpSinkServer::GetPlayingState(const RawAddress &device, int &state)
{
    ...
    return pimpl->a2dpSnkService_->GetPlayingState(realAddr, state);
}
...
int BluetoothA2dpSinkServer::SetConnectStrategy(const RawAddress &device, int strategy)
{
    ...
    return pimpl->a2dpSnkService_->SetConnectStrategy(realAddr, strategy);
}
...
int BluetoothA2dpSinkServer::GetConnectStrategy(const RawAddress &device)
{
    ...
    return pimpl->a2dpSnkService_->GetConnectStrategy(realAddr);
}
...
int BluetoothA2dpSinkServer::SendDelay(const RawAddress &device, int32_t delayValue)
{
    ...
    return pimpl->a2dpSnkService_->SendDelay(realAddr, (uint16_t)delayValue);
}
}  // namespace Bluetooth
}  // namespace OHOS