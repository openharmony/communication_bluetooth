...
public:
     ...
     */
    ~HfpAgStateMachine() override = default;
    ...
    bool ProcessConnectAudioEvent();
    bool ProcessDisconnectAudioEvent();
    void ProcessOpenRecognitionEvent(int requestSource);
    void ProcessCloseRecognitionEvent();
    void ProcessKeyPressed(const RawAddress &device, const int &callState);
    std::string GetStateName(int state) const;
    int GetPreState() const;
    bool GetTryReconnectHfp() const;
    void SetTryReconnectHfp(bool tryToReconnect);
    void SetAudioParameters();
    void SendAtCommandByEventProcessor(AppCategory topApptype);
...
private:
    void ProcessAudioStateChanged(const std::string &address, int fromState, int toState);
    ...
    inline static const int CONNECTION_TIMEOUT_MS {30000};  // 30s
...
public:
    ...
    HfpAgState(HfpAgSmInfo stu, HfpAgSystemEventProcesser &eventDispatch,
        int stateInt, utility::StateMachine::State &parent)
    ...
    HfpAgState(const std::string &name, utility::StateMachine &statemachine, HfpAgSystemEventProcesser &eventDispatch,
        int stateInt)
        : State(name, statemachine),
        eventProcessor_(eventDispatch),
        stateInt_(stateInt),
        stateMachine_(reinterpret_cast<HfpAgStateMachine &>(statemachine))
    {}
    ~HfpAgState() override
    {}
    int GetStateInt() const
    ...
    void ProcessClccTimeoutRsp() const;
    void ProcessTransition(const std::string &name);
...
public:
    HfpAgDisconnected(const std::string &name, utility::StateMachine &statemachine,
        HfpAgSystemEventProcesser &eventDispatch)
        : HfpAgState(name, statemachine, eventDispatch, HFP_AG_STATE_DISCONNECTED)
    {}
...
public:
    HfpAgConnecting(const std::string &name, utility::StateMachine &statemachine,
        HfpAgSystemEventProcesser &eventDispatch)
        : HfpAgState(name, statemachine, eventDispatch, HFP_AG_STATE_CONNECTING)
    {}
    ~HfpAgConnecting() = default;
    virtual void Entry() override;
    virtual void Exit() override;
    virtual bool Dispatch(const utility::Message &msg) override;
    bool ProcessConnectionStateChangedEvent(int connectState);
};

class HfpAgDisconnecting : public HfpAgState {
public:
    HfpAgDisconnecting(const std::string &name, utility::StateMachine &statemachine,
        HfpAgSystemEventProcesser &eventDispatch)
        : HfpAgState(name, statemachine, eventDispatch, HFP_AG_STATE_DISCONNECTING)
    {}
...
public:
    HfpAgConnected(const std::string &name, utility::StateMachine &statemachine,
        HfpAgSystemEventProcesser &eventDispatch)
        : HfpAgState(name, statemachine, eventDispatch, HFP_AG_STATE_CONNECTED)
    {}
    ...
    {}
    virtual bool Dispatch(const utility::Message &msg) override;
    void ProcessOpenRecognitionResultEvent(int result);
    void ProcessConnectionStateChanged(const HfpAgMessage &event);
    void ProcessAudioConnectionStateChanged(const HfpAgMessage &event);
    void DispatchOtherMsg(const HfpAgMessage &event);
private:
};

class HfpAgAudioConnecting : public HfpAgState {
public:
    HfpAgAudioConnecting(const std::string &name, utility::StateMachine &statemachine,
        HfpAgSystemEventProcesser &eventDispatch, utility::StateMachine::State &parent)
        : HfpAgState(HfpAgSmInfo(name, statemachine), eventDispatch, HFP_AG_AUDIO_STATE_CONNECTING, parent)
    {}
...
public:
    HfpAgAudioDisconnecting(const std::string &name, utility::StateMachine &statemachine,
        HfpAgSystemEventProcesser &eventDispatch, utility::StateMachine::State &parent)
        : HfpAgState(HfpAgSmInfo(name, statemachine), eventDispatch, HFP_AG_AUDIO_STATE_DISCONNECTING, parent)
    {}
...
public:
    HfpAgAudioConnected(const std::string &name, utility::StateMachine &statemachine,
        HfpAgSystemEventProcesser &eventDispatch, utility::StateMachine::State &parent)
        : HfpAgState(HfpAgSmInfo(name, statemachine), eventDispatch, HFP_AG_AUDIO_STATE_CONNECTED, parent)
    {}
