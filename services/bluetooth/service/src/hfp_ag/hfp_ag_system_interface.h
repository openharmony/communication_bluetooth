...
public:
     ...
     */
    int32_t DialOutCall(const std::string &address, const std::string &number) const;
     ...
     * @return Returns <b>true</b> if the operation is successful; returns <b>false</b> if the operation fails.
     */
    bool HoldCall(int chld, const std::string &address = "") const;
     ...
     * @return Returns <b>true</b> if the operation is successful; returns <b>false</b> if the operation fails.
     */
    void SetAudioParameters(const std::string &key, const std::string &value) const;

    /**
     * @brief Set the extra parameter to the audio.
     *
     * @param parameters Audio extra parameters.
     * @return Returns <b>true</b> if the operation is successful; returns <b>false</b> if the operation fails.
     */
    void SetExtraAudioParameters(const std::string &key,
    ...
    /**
     * @brief Mute incoming call.
     *
     * @return Returns indicates whether to initiate mute.
     */
    bool MuteRinger() const;
     ...
     */
    bool HandleChld(int chld, const std::string &address = "") const;
    ...
    /**
     * @brief process system ability added
     *
    ...
    /**
     * @brief process system ability removed
     *
     * @param systemAbilityId system ability id
     */
    void OnRemoveSystemAbility(int32_t systemAbilityId);

    void RemoveClccIndex(int callId);

    bool IsForegroundCallExist();
...
private:
    ...
    /**
     * @brief Notify the state changed to the device.
     */
    void SendDeviceStateChanged() const;
    ...
    void ProcessConnectTwoCall() const;

    int GetIndexForCall(int callId);

    int QuerySignalStrength() const;

#ifdef BLUETOOTH_HFP_AG_ENABLE
    /**
     * @brief Obtains the slot ID of the phone number when the Bluetooth peripheral makes a call.
     *
     * @param number The number of the call.
     * @return Returns the SIM card ID of the phone number. 0: SIM1; 1: SIM2.
     */
    int GetSlotId(const std::string &number) const;

    /**
     * @brief Check whether the card is in the normal state.
     *
     * @param slotId The SIM card ID.
     * @return Returns <b>true</b> Can be used normally; returns <b>false</b> The is abnormal or not activated.
     */
    bool IsSimCardAvailable(int32_t slotId) const;

    void ProcessAddCallToConversation(const Telephony::CallAttributeInfo &call) const;

    void ProcessReleaseHoldAccpetOther(const std::vector<Telephony::CallAttributeInfo> &callList,
        const std::string &address = "") const;

    void ProcessReleaseActiveAccpetOther(const Telephony::CallAttributeInfo &call) const;

    void ProcessReleaseAllHeldCalls(const Telephony::CallAttributeInfo &call) const;
#endif
    ...
    // A map from Calls to indexes used to identify calls for CLCC
    SafeMap<int, int> clccIndexMap_;

    BtRecursiveMutex agIndicatorMutex_ {};
