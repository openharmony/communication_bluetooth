...
#ifndef HFP_AG_RFCOMM_CONNECTION_H
#define HFP_AG_RFCOMM_CONNECTION_H
...
public:
    ...
    /**
     * @brief Construct a new HfpAgRfcommConnection object.
     *
     * @param fn Rfcomm callback.
     */
    explicit HfpAgRfcommConnection(RFCOMM_EventCallback fn) : fn_(fn)
    {}

    /**
     * @brief Destroy the HfpAgRfcommConnection object.
     */
...
private:
    ...
    std::unique_ptr<HfpAgGapClient> gap_ {std::make_unique<HfpAgGapClient>()};
    ...
    BT_DISALLOW_COPY_AND_ASSIGN(HfpAgRfcommConnection);
};
}  // namespace bluetooth
#endif // HFP_AG_RFCOMM_CONNECTION_H