...
#ifndef HFP_AG_AUDIO_CONNECTION_H
#define HFP_AG_AUDIO_CONNECTION_H
...
private:
     ...
     */
    static BtAddr ConvertToBtAddr(std::string address);
     ...
     */
    static std::vector<HfpAgAudioConnection::AudioDevice>::iterator GetDeviceByAddr(const std::string &addr);
     ...
     * @return Returns audio device or nullopt.
     */
    static std::vector<HfpAgAudioConnection::AudioDevice>::iterator GetDeviceByHandle(uint16_t handle);
     ...
     */
    int AcceptByCvsd(const AudioDevice &dev, BtAddr btAddr) const;

    /**
     * @brief Connect SCO by MSBC.
     *
     * @param dev Audio device.
     * @param btAddr Bt address.
     * @return Returns connect SCO by MSBC result.
     */
    int ConnectByMsbc(AudioDevice &dev, BtAddr btAddr) const;
    ...
    BT_DISALLOW_COPY_AND_ASSIGN(HfpAgAudioConnection);
    ...
    static inline constexpr int CVSD_SCO_MAX_LATENCY = 0x0000ffff;
