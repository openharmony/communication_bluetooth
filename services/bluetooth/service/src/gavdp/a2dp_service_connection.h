...
public:
    ...
    /**
     * @brief avdtp control with the peer bluetooth service.
     *
     * @param device The address of the peer bluetooth device.
     * @param event the a2dp stream control type by play or stop.
     * @return Returns <b>true</b> if the operation is successful.
     *         Returns <b>false</b> if the operation fails.
     * @since 6.0
     */
    bool A2dpStreamControl(const RawAddress &device, int event);
     ...
     * @since 6.0
     */
    std::shared_ptr<A2dpDeviceInfo> AddDevice(const RawAddress &device, int state);
    ...
    /**
     * @brief Received exception error from avdtp stack.
     *
     * @param device The address of the peer bluetooth device.
     * @param errCode exception error code occurred in avdtp.
     * @since 13.0
     */
    void ReceiveA2dpAbnormalError(const RawAddress &device, int32_t errCode);

    /**
     * @brief Remove a2dp abnormal virtual device.
     *
     * @param device The address of the peer bluetooth device.
     * @since 13.0
     */
    void RemoveA2dpAbnormalVirtualDevice(const RawAddress &device);

private:
    /**
     * @brief Handle a2dp abnormal error device.
     *
     * @param device The raw address of the bluetooth device.
     * @param errCode a2dp device abnormal error code.
     * @since 13.0
     */
    void HandleA2dpAbnormalError(const RawAddress &device, int32_t errCode);

private:
    // The a2dp profile role of local device.
    uint8_t role_ {};
    utility::SafeVector<std::string> abnomalDevices_ = {};
};
}  // namespace bluetooth
}  // namespace OHOS
