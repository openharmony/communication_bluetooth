...
static void WatchDisconnectRssiCb(const BLUEDROID::RawAddress* bdAddr, int rssi)
{
...
static void WatchSaisConnectionStateCb(const BLUEDROID::RawAddress* bdAddr, bool isSaisConnected)
{
...
static void HwWatchLeSaisEnableCb(const BLUEDROID::RawAddress* bdAddr)
{
...
static void HidConnectionStateCb(BLUEDROID::RawAddress* bdAddr, bthd_connection_state_t state)
{
    RawAddress rawAddr = ServiceUtil::AddrFromBluedroid(*bdAddr);
    auto deviceManager = GlobalDeviceManager::getInstance().GetSaisDevice();
...
void WatchService::UpdateReconnectState(bool isBtOn)
{
    ...
    for (const auto &address : addresses) {
        ...
        GetBluetoothWatchInterface()->updateReconnectState(ServiceUtil::AddrToBluedroid(device), isBtOn);
    }
}
...
void WatchService::UpdateSaisDevice(const RawAddress &device, bool isSaisDevice)
{
    ...
    GetBluetoothWatchInterface()->updateDevice(ServiceUtil::AddrToBluedroid(device), isSaisDevice);
}
...
void WatchService::UpdateHFStrategy(const RawAddress &device, int strategy)
{
    if (strategy == static_cast<int>(BTStrategyType::CONNECTION_FORBIDDEN)) {
        GetBluetoothWatchInterface()->updateReconnectState(ServiceUtil::AddrToBluedroid(device), false);
    } else {
        GetBluetoothWatchInterface()->updateReconnectState(ServiceUtil::AddrToBluedroid(device), true);
    }
}
...
void WatchService::SetSaisConnectionState(const RawAddress &device, bool isConnected)
{
    ...
    if (isConnected) {
        ...
        GetBluetoothWatchInterface()->updateDevice(ServiceUtil::AddrToBluedroid(device), true);
...
void WatchService::ProcessHfpStateChangeEvt(const RawAddress &device, int state)
{
    ...
    if (state == HFP_HF_STATE_CONNECTED) {
        ...
        GetBluetoothWatchInterface()->sendHfpState2Hisi(ServiceUtil::AddrToBluedroid(device), false);
...
{
    bool isPhone = GetBluetoothWatchInterface()->isPhoneMajorClass(ServiceUtil::AddrToBluedroid(device));
...
bool WatchService::ConnectHfp(const std::string& address)
{
    ...
    GetBluetoothWatchInterface()->sendHfpState2Hisi(ServiceUtil::AddrToBluedroid(device), true);
...
void WatchService::HandleVoipState(int voipState)
{
    ...
    if (voipState == VOIP_CALL_INCOMMING) {
        ...
        GetBluetoothWatchInterface()->sendTbsr(ServiceUtil::AddrToBluedroid(device), HEADPHONE_MODE);
        ...
        GetBluetoothWatchInterface()->sendTbsr(ServiceUtil::AddrToBluedroid(device), BAND_MODE);
