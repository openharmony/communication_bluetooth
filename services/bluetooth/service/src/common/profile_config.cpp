...
bool ProfileConfig::GetValue(const std::string &section, const std::string &property, std::string &value)
{
    std::lock_guard<std::mutex> lock(pimpl->mutex_);
...
bool ProfileConfig::SetValue(const std::string &section, const std::string &property, const std::string &value)
{
    std::lock_guard<std::mutex> lock(pimpl->mutex_);
    if (!pimpl->parse_.SetValue(section, property, value)) {
        return false;
    }
    return pimpl->parse_.Save();
}
