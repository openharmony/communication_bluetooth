...
public:
     ...
     * @since 6
     */
    ~ProfileServiceManager() override;
...
private:
    ...
    bool NeedWaitOtherProfileComplete(const std::string &name) const;
    void CheckEnableOtherProfile(const std::string &enabledProfile) const;
    ...
    /**
     * @brief Set profile service, only available for TDD use cases.
     *
     * @param transport Profile service name.
     * @param name transport Adapter transport.
     */
    void SetProfileService(const BTTransport transport, const std::string &name);
