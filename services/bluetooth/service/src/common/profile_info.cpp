...
const std::vector<ProfileInfo> SupportProfilesInfo::SUPPORT_FILES = {
    ...
    ProfileInfo(PROFILE_NAME_HID_DEVICE, PROFILE_ID_HID_DEVICE, BLUETOOTH_UUID_HID_DEVICE),
    ...
    ProfileInfo(PROFILE_NAME_HEARING_AID, PROFILE_ID_HEARING_AID, BLUETOOTH_UUID_HEARING_AID),
#ifdef BLUETOOTH_BAS_FEATURE_ENABLE
    ProfileInfo(PROFILE_NAME_BAS, PROFILE_ID_BAS, BLUETOOTH_UUID_BAS),
#endif
};
...
const std::vector<ProfileInfo> SupportProfilesInfo::GetConfigSupportProfiles(BTTransport transport)
{
    ...
    for (auto &sp : SUPPORT_FILES) {
        ...
        } else if (sp.name_.compare(PROFILE_NAME_HFP_HF) == 0) {
#ifdef BLUETOOTH_HFP_HF_ENABLE
            retProfiles.push_back(sp);
#endif
        } else if (sp.name_.compare(PROFILE_NAME_A2DP_SINK) == 0 && transport == BTTransport::ADAPTER_BREDR) {
