...
{
    LOG_DEBUG("[HFP HF]%{public}s(): Audio Register start", __FUNCTION__);
...
{
    LOG_DEBUG("[HFP HF]%{public}s(): Audio Deregister start", __FUNCTION__);
    std::vector<HfpHfAudioConnection::AudioDevice>().swap(g_audioDevices);
...
void HfpHfAudioConnection::SetSupportFeatures(bool escoSupport, bool escoS4Support, int inUseCodec)
{
    ...
    LOG_DEBUG("[HFP HF]%{public}s(): escoSupport_[%{public}d], escoS4Support_[%{public}d], inUseCodec_[%{public}d]",
        __FUNCTION__,
        escoSupport_,
        escoS4Support_,
        inUseCodec_);
...
int HfpHfAudioConnection::ConnectByCvsd(AudioDevice &dev, BtAddr btAddr, bool cvsdEscoFailed) const
{
    ...
    if (escoSupport_ && !cvsdEscoFailed) {
        ...
        if (escoS4Support_) {
            LOG_DEBUG("[HFP HF]%{public}s():Try connect by CVSD ESCO S4.", __FUNCTION__);
            ...
            LOG_DEBUG("[HFP HF]%{public}s():Try connect by CVSD ESCO S1.", __FUNCTION__);
        ...
        if (!BTM_IsSecureConnection(&btAddr)) {
            ...
            LOG_DEBUG("[HFP HF]%{public}s():Try connect by CVSD SCO.", __FUNCTION__);
...
{
    LOG_DEBUG("[HFP HF]%{public}s():Connect SCO to %{public}s", __FUNCTION__, remoteAddr_.c_str());
    ...
    if (dev == g_audioDevices.end()) {
        ...
        LOG_DEBUG("[HFP HF]%{public}s():Create Audio device for [%{public}s]", __FUNCTION__, remoteAddr_.c_str());
    } else {
        LOG_DEBUG("[HFP HF]%{public}s():Audio device [%{public}s] already in device list ", __FUNCTION__, remoteAddr_.c_str());
    }
...
{
    LOG_DEBUG("[HFP HF]%{public}s():Disconnect SCO from %{public}s", __FUNCTION__, remoteAddr_.c_str());
...
int HfpHfAudioConnection::AcceptByMsbc(BtAddr btAddr)
{
    ...
    if (BTM_IsSecureConnection(&btAddr)) {
        LOG_DEBUG("[HFP HF]%{public}s():Accept by MSBC T2.", __FUNCTION__);
        BtmCreateEscoConnectionParam param = MSBC_T2_PARAM;
        param.addr = btAddr;
        ret = BTM_AcceptEscoConnectionRequest(&param);
        HFP_HF_RETURN_IF_FAIL(ret);
    } else {
        LOG_DEBUG("[HFP HF]%{public}s():Accept by MSBC T1.", __FUNCTION__);
...
int HfpHfAudioConnection::AcceptByCvsd(AudioDevice &dev, BtAddr btAddr) const
{
    ...
    if (dev.linkType == LINK_TYPE_ESCO && escoSupport_) {
        if (escoS4Support_) {
            LOG_DEBUG("[HFP HF]%{public}s():Accept by CVSD ESCO S4.", __FUNCTION__);
            BtmCreateEscoConnectionParam param = CVSD_ESCO_S4_PARAM;
            param.addr = btAddr;
            ret = BTM_AcceptEscoConnectionRequest(&param);
            HFP_HF_RETURN_IF_FAIL(ret);
        } else {
            LOG_DEBUG("[HFP HF]%{public}s():Accept by CVSD ESCO S1.", __FUNCTION__);
            BtmCreateEscoConnectionParam param = CVSD_ESCO_S1_PARAM;
            param.addr = btAddr;
            ret = BTM_AcceptEscoConnectionRequest(&param);
            HFP_HF_RETURN_IF_FAIL(ret);
        ...
        LOG_DEBUG("[HFP HF]%{public}s():Accept by CVSD SCO.", __FUNCTION__);
        BtmCreateScoConnectionParam param = CVSD_SCO_PARAM;
        param.addr = btAddr;
    ...
    if (dev != g_audioDevices.end()) {
        if (inUseCodec_ == HFP_HF_CODEC_MSBC) {
            if (dev->linkType == LINK_TYPE_ESCO && escoSupport_) {
                ...
                LOG_DEBUG("[HFP HF]%{public}s():Accpet MSBC ESCO connection failed, linktype[%hhu] and escoSupport[%{public}d] "
                          "are not matched!",
                    __FUNCTION__,
                    dev->linkType,
                    escoSupport_);
            ...
            LOG_DEBUG("[HFP HF]%{public}s():Invalid Codec[%{public}d]!", __FUNCTION__, inUseCodec_);
            return BT_BAD_PARAM;
        }
    } else {
        LOG_WARN("[HFP HF]%{public}s:Invalid Address", remoteAddr_.c_str());
...
{
    LOG_DEBUG("[HFP HF]%{public}s():Reject sco connect request from %{public}s", __FUNCTION__, remoteAddr_.c_str());
    BtAddr btAddr = ConvertToBtAddr(remoteAddr_);
...
void HfpHfAudioConnection::ProcessOnConnectRequest(HfpScoConnectionRequestParam parameters)
{
    ...
    if (dev == g_audioDevices.end()) {
        ...
        g_audioDevices.push_back(audioDev);
        LOG_DEBUG("[HFP HF]%{public}s():Create Audio device for [%{public}s]", __FUNCTION__, address.c_str());
    } else {
        dev->linkType = parameters.linkType;
        LOG_DEBUG("[HFP HF]%{public}s():Audio device [%{public}s] already in device list ", __FUNCTION__, address.c_str());
    }
...
void HfpHfAudioConnection::ProcessOnConnectCompleted(HfpScoConnectionCompleteParam parameters)
{
    ...
    if (dev != g_audioDevices.end()) {
        ...
        if (!parameters.status) {
            LOG_DEBUG("[HFP HF]%{public}s(): SCO connect successfully!", __FUNCTION__);
        ...
        LOG_DEBUG("[HFP HF]%{public}s:Invalid audio device", address.c_str());
    }
}
    ...
    std::vector<HfpHfAudioConnection::AudioDevice>::iterator dev, const std::string &address)
{
    ...
    if (dev->role == ROLE_INITIATOR) {
        if (dev->lastParam == CVSD_ESCO_S4 || dev->lastParam == CVSD_ESCO_S1) {
            LOG_DEBUG("[HFP HF]%{public}s():CVSD ESCO failed, try CVSD SCO", __FUNCTION__);
            ...
            LOG_DEBUG("[HFP HF]%{public}s():CVSD SCO failed, report fail event to service", __FUNCTION__);
        ...
        HfpHfProfileEventSender::GetInstance().UpdateScoConnectState(dev->addr, HFP_HF_AUDIO_CONNECT_FAILED_EVT);
        LOG_DEBUG("[HFP HF]%{public}s(): Accept SCO from address[%{public}s]failed", __FUNCTION__, address.c_str());
    }
}
...
void HfpHfAudioConnection::ProcessOnDisconnectCompleted(HfpScoDisconnectionCompleteParam parameters)
{
    ...
    if (it != g_audioDevices.end()) {
        if (!parameters.status) {
            LOG_DEBUG("[HFP HF]%{public}s(): Disconnect SCO from address[%{public}s] successfully.", __FUNCTION__, it->addr.c_str());
            HfpHfProfileEventSender::GetInstance().UpdateScoConnectState(it->addr, HFP_HF_AUDIO_DISCONNECTED_EVT);
            g_audioDevices.erase(it);
        } else {
            LOG_DEBUG("[HFP HF]%{public}s(): Disconnect SCO from address[%{public}s] failed.", __FUNCTION__, it->addr.c_str());
...
{
    LOG_DEBUG("[HFP HF]%{public}s():", __PRETTY_FUNCTION__);
    LOG_DEBUG("[HFP HF]%{public}s():connectionHandle[%hu]", __FUNCTION__, param->connectionHandle);
}

void HfpHfAudioConnection::OnWriteVoiceSettingCompleted(uint8_t status, void *context)
{
    LOG_DEBUG("[HFP HF]%{public}s():", __PRETTY_FUNCTION__);
    LOG_DEBUG("[HFP HF]%{public}s(): status[%hhu]", __FUNCTION__, status);
}
}  // namespace bluetooth