...
#ifndef GATT_SERVICE_OVER_BREDR_H
#define GATT_SERVICE_OVER_BREDR_H
...
public:
    ...
    virtual ~GattServiceOverBredrInterface() {}
};

class GattServiceOverBredr : public GattServiceOverBredrInterface {
public:
    GattServiceOverBredr() {}
    ~GattServiceOverBredr() {}

    int RegisterSDP(uint16_t startHandle, uint16_t endHandle) override;
    void DeregisterSDP() override;

    BT_DISALLOW_COPY_AND_ASSIGN(GattServiceOverBredr);
private:
