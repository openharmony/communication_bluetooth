...
void GattServiceOverBredr::DeregisterSDP()
{
    LOG_DEBUG("[GenericAttributeService]::%{public}s", __FUNCTION__);
...
int GattServiceOverBredr::SdpAddServiceClassIdList() const
{
    BtUuid classid;
    classid.type = BT_UUID_16;
    classid.uuid16 = UUID_GENERIC_ATTRIBUTE_SERVICE;
    return SDP_AddServiceClassIdList(sdpHandle_, &classid, 1);
}

int GattServiceOverBredr::SdpAddProtocol(uint16_t startHandle, uint16_t endHandle) const
{
    static const size_t gattsProtocolNum = 2;
    static const size_t gattProtParmNum = 2;
    static const uint16_t gattPsmAtt = 0x001F;
    SdpProtocolDescriptor protocol[gattsProtocolNum];
    ...
    return SDP_AddProtocolDescriptorList(sdpHandle_, protocol, gattsProtocolNum);
}

int GattServiceOverBredr::SdpAddBrowseGroupList() const
{
