...
public:
     ...
     * @param ret Result of the callback.
     * @param characteristic Characteristic object.
     * @param rspContext BluetoothGattRspContext object.
     * @since 23
     *
     */
    virtual void OnCharacteristicWrite(int ret, const Characteristic &characteristic,
        const Bluetooth::BluetoothGattRspContext &rspContext) = 0;
    /**
     ...
     *
     */
    virtual void OnConnectionStateChanged(int state, int newState, const RawAddress &addr, int disconnectReason = -1,
        const std::string &reasonMessage = "") = 0;
    /**
...
public:
     ...
     *
     */
    virtual int RegisterApplication(std::weak_ptr<IGattClientCallback> callback,
        const RawAddress &addr, uint8_t transport) = 0;

    virtual int RegisterSharedApplication(std::weak_ptr<IGattClientCallback> callback,
        const RawAddress &addr, uint8_t transport) = 0;
    /**
    ...
    virtual int ReadCharacteristicByUuid(int appId, const Uuid &uuid,
    ...
    virtual int RequestFastestConn(const RawAddress &addr) = 0;

    virtual int ReadRemoteRssiValue(int appId) = 0;

    /**
     * @brief The function to request notify characteristic chanage.
     * @param appId Application id.
     * @param characterhandle characteristic handle.
     * @param enable enbale.
     *
     * @return int api accept status.
     * @since 6
    ...
    virtual int GetConnectedState(const RawAddress &address) = 0;
