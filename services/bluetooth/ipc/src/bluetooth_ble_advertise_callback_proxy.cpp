 ...
 */
#ifndef LOG_TAG
#define LOG_TAG "bt_ipc_ble_advertise_cb_proxy"
#endif
...
void BluetoothBleAdvertiseCallbackProxy::OnStopResultEvent(int32_t result, int32_t advHandle)
{
    ...
    MessageOption option = {MessageOption::TF_ASYNC};
    int error = InnerTransact(
        BluetoothBleAdvertiseCallbackInterfaceCode::BT_BLE_ADVERTISE_CALLBACK_STOP_RESULT_EVENT, option, data, reply);
    if (error != NO_ERROR) {
        HILOGE("failed, error: %{public}d", error);
        return;
    }
    ...
    bluetooth::SetAdvDataType type)
{
    MessageParcel data;
    if (!data.WriteInterfaceToken(BluetoothBleAdvertiseCallbackProxy::GetDescriptor())) {
        HILOGE("[OnSetAdvDataEvent] fail: write interface token failed.");
        return;
    }

    if (!data.WriteInt32(result)) {
        HILOGE("[OnSetAdvDataEvent] fail: write result failed");
        return;
    }

    if (!data.WriteInt32(advHandle)) {
        HILOGE("[OnSetAdvDataEvent] fail: write advHandle failed");
        return;
    }

    if (!data.WriteInt32(type)) {
        HILOGE("[OnSetAdvDataEvent] fail: write type failed");
        return;
    }
    ...
    MessageOption option = {MessageOption::TF_ASYNC};
    int error = InnerTransact(
        BluetoothBleAdvertiseCallbackInterfaceCode::BT_BLE_ADVERTISE_CALLBACK_SET_ADV_DATA, option, data, reply);
    if (error != NO_ERROR) {
        HILOGE("failed, error: %{public}d", error);
        return;
    }
...
void BluetoothBleAdvertiseCallbackProxy::OnChangeAdvResultEvent(int32_t result, int32_t advHandle)
{
    ...
    MessageParcel reply;
    MessageOption option = { MessageOption::TF_ASYNC };
    int error = InnerTransact(BluetoothBleAdvertiseCallbackInterfaceCode::BT_BLE_ADVERTISE_CALLBACK_CHANGE_ADV_RESULT,
        option, data, reply);
    if (error != NO_ERROR) {
        HILOGE("failed, error: %{public}d", error);
        return;
