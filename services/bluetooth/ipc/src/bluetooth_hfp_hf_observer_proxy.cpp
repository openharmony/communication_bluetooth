 ...
 */
#ifndef LOG_TAG
#define LOG_TAG "bt_ipc_hfp_hf_observer_proxy"
#endif
...
void BluetoothHfpHfObserverProxy::OnOperatorSelectionChanged(
    const BluetoothRawAddress &device, const std::string &name)
{
    MessageParcel data;
    if (!data.WriteInterfaceToken(BluetoothHfpHfObserverProxy::GetDescriptor())) {
...
void BluetoothHfpHfObserverProxy::OnSubscriberNumberChanged(
    const BluetoothRawAddress &device, const std::string &number)
{
    MessageParcel data;
    if (!data.WriteInterfaceToken(BluetoothHfpHfObserverProxy::GetDescriptor())) {
