 ...
 */
#ifndef LOG_TAG
#define LOG_TAG "bt_ipc_ble_central_manag_cb_proxy"
#endif
    ...
    const std::vector<uint8_t> &notifyValue)
{
    MessageParcel data;
    if (!data.WriteInterfaceToken(BluetoothBleCentralManagerCallBackProxy::GetDescriptor())) {
        HILOGE("[OnNotifyMsgReportFromLpDevice] write interface token failed.");
        return;
    }
    BluetoothUuid btUuid(uuid);
    if (!data.WriteParcelable(&btUuid)) {
        HILOGE("[OnNotifyMsgReportFromLpDevice] write uuid failed");
        return;
    }
    if (!data.WriteInt32(msgType)) {
        HILOGE("[OnNotifyMsgReportFromLpDevice] write msgType failed");
        return;
    }
    if (!data.WriteUInt8Vector(notifyValue)) {
        HILOGE("[OnNotifyMsgReportFromLpDevice] write notifyValue failed.");
        return;
    }
