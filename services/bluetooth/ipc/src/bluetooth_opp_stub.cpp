...
namespace OHOS {
namespace Bluetooth {
const uint32_t OPP_FILE_PATH_SIZE_MAX = 0XFF;
const uint32_t OPP_MIME_SEND_SIZE_MAX = 0XFF;
const uint32_t OPP_DEVICE_BY_STATES_SIZE_MAX = 0XFF;
...
ErrCode BluetoothOppStub::SendFileInner(MessageParcel &data, MessageParcel &reply)
{
    ...
    if (static_cast<unit32_t>(fileSize) > OPP_FILE_PATH_SIZE_MAX) {
    ...
    if (static_cast<unit32_t>(mimeSize) > OPP_MIME_SEND_SIZE_MAX) {
        return BT_ERR_INTERNAL_ERROR;
    }
...
ErrCode BluetoothOppStub::GetDevicesByStatesInner(MessageParcel &data, MessageParcel &reply)
{
    ...
    if (static_cast<unit32_t>(stateSize) > OPP_DEVICE_BY_STATES_SIZE_MAX) {
        return BT_ERR_INTERNAL_ERROR;
    }
    HILOGI("BluetoothOppStub::GetDevicesByStatesInner");
