...
int32_t BtHciCallbacksStub::OnRemoteRequest(uint32_t code,
...
int32_t BtHciCallbacksStub::BtHciCallbacksStubOnReceivedHciPacket(MessageParcel& data, MessageParcel& reply, MessageOption& option)
{
    BtType type = (BtType)data.ReadUint32();

    std::vector<int8_t> raw;
    uint32_t dataSize = data.ReadUint32();
    for (uint32_t i0 = 0; i0 < dataSize; ++i0) {
        int8_t value0 = (int8_t)data.ReadInt8();
        raw.push_back(value0);
    }
