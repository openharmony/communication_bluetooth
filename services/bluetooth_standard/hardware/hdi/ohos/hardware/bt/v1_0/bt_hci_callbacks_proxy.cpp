...
int32_t BtHciCallbacksProxy::OnReceivedHciPacket(BtType type, const std::vector<int8_t>& data)
{
    ...
    if (!localData.WriteUint32(data.size())) {
        HDF_LOGE("%s: write data.size() failed!", __func__);
        return HDF_ERR_INVALID_PARAM;
    }
    for (auto it0 : data) {
        if (!localData.WriteInt8(it0)) {
            HDF_LOGE("%s: write it0 failed!", __func__);
            return HDF_ERR_INVALID_PARAM;
        }
    }
