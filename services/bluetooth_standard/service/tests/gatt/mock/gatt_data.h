...
struct Descriptor {
    ...
    Descriptor(Descriptor&& src)
        : handle_(src.handle_), permissions_(src.permissions_), value_(std::move(src.value_)), length_(src.length_), uuid_(src.uuid_)
    {
        src.value_ = nullptr;
        src.length_ = 0;
    }

    Descriptor(uint16_t handle) : handle_(handle), permissions_(0), value_(nullptr), length_(0), uuid_() {}
...
struct Characteristic {
    ...
    Characteristic(uint16_t handle) 
    ...
    Characteristic(Characteristic&& src)
        ...
        properties_(src.properties_), permissions_(src.permissions_), value_(std::move(src.value_)), length_(src.length_),
        uuid_(src.uuid_), descriptors_(std::move(src.descriptors_))
    {
        src.value_ = nullptr;
        src.length_ = 0;
    }
