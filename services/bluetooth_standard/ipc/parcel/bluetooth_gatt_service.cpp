...
bool BluetoothGattService::Marshalling(Parcel &parcel) const {
    ...
    for(int j = 0; j < cha_num; j++) {
        ...
        if (!parcel.WriteUint32(characteristics_[j].uuid_.ConvertTo32Bits())) {
            return false;
        }
        ...
        for(l = 0; l < des_num; l++) {
            ...
            if (!parcel.WriteUint32(characteristics_[j].descriptors_[l].uuid_.ConvertTo32Bits())) {
                return false;
            }
        }
    }
    ...
    if (parcel.ReadInt32(cha_num)) {
        for(int z = 0; z < cha_num; z++) {
            ...
            bluetooth::Characteristic characteristic = bluetooth::Characteristic(
                bluetooth::Uuid::ConvertFrom32Bits(cha_uuid), cha_handle, 
                cha_properties, cha_permissions, value_v, cha_length);
            ...
            if (parcel.ReadInt32(des_num)) {
                ...
                for(int des_j = 0; des_j < des_num; des_j++) {
                    ...
                    bluetooth::Descriptor desc = bluetooth::Descriptor(
                        bluetooth::Uuid::ConvertFrom32Bits(des_uuid), des_handle, 
                        des_permissions, des_value, des_length);
        ...
        for(int z = 0; z < cha_num; z++) {
            ...
            if (parcel.ReadInt32(des_num)) {
                ...
                for(int des_j = 0; des_j < des_num; des_j++) {
                    ...
                    bluetooth::Descriptor desc = bluetooth::Descriptor(
                        bluetooth::Uuid::ConvertFrom32Bits(des_uuid), des_handle, 
                        des_permissions, par_value, des_length);
...
bool BluetoothGattService::ServiceIteratorWrite(const std::vector<Service> &service, Parcel &parcel, int length) const{
    for(int i = 0; i < length; i++) {
        ...
        if (!parcel.WriteUint32(service[i].uuid_.ConvertTo32Bits())) {
            return false;
        }
        ...
        for(int j = 0; j < cha_num; j++) {
            ...
            if (!parcel.WriteUint32(service[i].characteristics_[j].uuid_.ConvertTo32Bits())) {
                return false;
            }
            ...
            for(l = 0; l < des_num; l++) {
                ...
                if (!parcel.WriteUint32(service[i].characteristics_[j].descriptors_[l].uuid_.ConvertTo32Bits())) {
                    return false;
                }
            }
        }
...
bool BluetoothGattService::ServiceIteratorRead(std::vector<Service> &service, Parcel &parcel, int length) {
    ...
    for(int i = 0; i < length; i++) {
        ...
        uint32_t uuid;
        if (!parcel.ReadUint32(uuid)) {
            HILOGI("BluetoothGattService::uuid: start");
            noError = false;
        }
        bluetooth::Service serv = bluetooth::Service(
                bluetooth::Uuid::ConvertFrom32Bits(uuid), handle, startHandle, endHandle);
