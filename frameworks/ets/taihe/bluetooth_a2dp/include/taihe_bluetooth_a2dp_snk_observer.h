...
#ifndef TAIHE_BLUETOOTH_A2DP_SNK_OBSERVER_H_
#define TAIHE_BLUETOOTH_A2DP_SNK_OBSERVER_H_
...
class TaiheA2dpSinkObserver : public A2dpSinkObserver {
public:
    void OnConnectionStateChanged(const BluetoothRemoteDevice &device, int state, int cause) override;

    TaiheA2dpSinkObserver();
    ~TaiheA2dpSinkObserver() override = default;

private:
    friend class A2dpSinkProfileImpl;
    EventModule<void(::ohos::bluetooth::baseProfile::StateChangeParam const& data)> eventSubscribe_;
};

}  // namespace Bluetooth
}  // namespace OHOS
#endif /* TAIHE_BLUETOOTH_A2DP_SNK_OBSERVER_H_ */