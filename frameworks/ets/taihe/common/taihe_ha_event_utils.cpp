...
#ifndef LOG_TAG
#define LOG_TAG "bt_taihe_ha_event_utils"
#endif
...
int64_t TaiheHaEventUtils::processorId_ = -1;
std::mutex TaiheHaEventUtils::processorLock_;
SafeMap<ani_env*, int32_t> TaiheHaEventUtils::envErrCodeMap_ = {};

TaiheHaEventUtils::TaiheHaEventUtils(ani_env *env, const std::string &apiName): env_(env), apiName_(apiName)
{
    beginTime_ = GetNowTimeMs();
    GenerateProcessorId();
}

TaiheHaEventUtils::~TaiheHaEventUtils()
{
    WriteEndEvent();
}

void TaiheHaEventUtils::WriteErrCode(ani_env *env, const int32_t errCode)
{
    envErrCodeMap_.EnsureInsert(env, errCode);
}

void TaiheHaEventUtils::WriteErrCode(const int32_t errCode)
{
    TaiheHaEventUtils::WriteErrCode(env_, errCode);
}

void TaiheHaEventUtils::GenerateProcessorId()
{
...
int64_t TaiheHaEventUtils::GetNowTimeMs() const
{
#ifndef CROSS_PLATFORM
    struct timespec ts = {};
...
int64_t TaiheHaEventUtils::AddProcessor()
{
#ifndef CROSS_PLATFORM
...
std::string TaiheHaEventUtils::RandomTransId() const
{
    return std::string("transId_") + std::to_string(std::rand());
}

void TaiheHaEventUtils::WriteEndEvent() const
{
#ifndef CROSS_PLATFORM
