...
namespace OHOS {
namespace Bluetooth {
std::shared_mutex NapiPbapPseObserver::g_pbapPseCallbackInfosMutex;
void NapiPbapPseObserver::OnConnectionStateChanged(const BluetoothRemoteDevice &device, int32_t state)
{
    HILOGD("enter, remote device address: %{public}s, state: %{public}d", GET_ENCRYPT_ADDR(device), state);
    std::unique_lock<std::shared_mutex> guard(g_pbapPseCallbackInfosMutex);

    std::map<std::string, std::shared_ptr<BluetoothCallbackInfo>>::iterator it =
        callbackInfos_.find(STR_BT_PBAP_PSE_CONNECTION_STATE_CHANGE);
