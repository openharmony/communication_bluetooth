...
struct BleAdvertiser::impl {
    ...
    uint32_t GetAdvertiseNameLen(const BluetoothBleAdvertiserData &data);
    ...
    std::mutex advInitLock = {};
    std::condition_variable advInitCV = {};
};
...
uint32_t BleAdvertiser::impl::GetAdvertiseNameLen(const BluetoothBleAdvertiserData &data)
{
    uint32_t advNameLen = 0;
    if (data.HasAdvertiseName()) { // if includeDeviceName is true and advertiseName is given, we use advertiseName
        advNameLen = data.GetAdvertiseName().length();
        advNameLen = ((advNameLen > DEVICE_NAME_MAX_LEN) ? DEVICE_NAME_MAX_LEN : advNameLen);
        advNameLen += BLE_ADV_PER_FIELD_OVERHEAD_LENGTH;
        return advNameLen;
    }
...
void BleAdvertiser::impl::WaitForAdvCallbackRegistering()
{
    ...
    if (callbackImp_ == nullptr) {
        HILOGE("error, null callbackImp_");
        return;
    }
    if (callbackImp_->isRegistered_) {
        HILOGD("BleAdvertiser callbackImp_ is registered");
        return;
    }
    HILOGI("BleAdvertiser callback not registered, waiting for registering");
    if (!advInitCV.wait_for(lock, std::chrono::milliseconds(BLE_ADV_INIT_TIMEOUT),
        [this]() { return callbackImp_->isRegistered_; })) {
        HILOGW("BleAdvertiser callbackImp_ register timeout");
        return;
    }
}
...
int BleAdvertiser::GetAdvPowerForRangingBusiness(BleApplicationType appType)
{
    CHECK_AND_RETURN_LOG_RET(IS_BLE_ENABLED(), BT_ERR_INVALID_STATE, "bluetooth is off.");
    sptr<IBluetoothBleAdvertiser> proxy = GetRemoteProxy<IBluetoothBleAdvertiser>(BLE_ADVERTISER_SERVER);
    CHECK_AND_RETURN_LOG_RET(proxy != nullptr, BT_ERR_INTERNAL_ERROR, "failed: no proxy");

    int8_t advpower = BLE_RANGING_INVALID_ADVPOWER;
    int ret = proxy->GetAdvPowerForRangingBusiness(static_cast<bluetooth::BleAppType>(appType), advpower);
    if (ret != BT_NO_ERROR) {
        HILOGE("proxy fail, ret: %{public}d", ret);
    }
    return advpower;
}

int BleAdvertiser::BleRestoreRangingAntSwtich(BleApplicationType appType)
{
    CHECK_AND_RETURN_LOG_RET(IS_BLE_ENABLED(), BT_ERR_INVALID_STATE, "bluetooth is off.");
    sptr<IBluetoothBleAdvertiser> proxy = GetRemoteProxy<IBluetoothBleAdvertiser>(BLE_ADVERTISER_SERVER);
    CHECK_AND_RETURN_LOG_RET(proxy != nullptr, BT_ERR_INTERNAL_ERROR, "failed: no proxy");

    int ret = proxy->BleRestoreRangingAntSwtich(static_cast<bluetooth::BleAppType>(appType));
    return ret;
}
...
bool BleAdvertiserData::HasAdvertiseName() const
{
