...
public:
    ...
    void OnStateChanged(int32_t transport, int32_t status) override
    {
        ...
        GetInstance().NotifyBluetoothStateChange(transport, status);
...
BluetoothHost::impl::impl()
{
    ...
    profileRegisterId = GetInstance().RegisterFunc(BLUETOOTH_HOST,
...
bool BluetoothHost::impl::LoadBluetoothHostService()
{
    ...
    sptr<IRemoteObject> hostRemote =
        GetInstance().GetProfileRemote(BLUETOOTH_HOST);
