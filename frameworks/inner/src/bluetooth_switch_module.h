...
public:
    ...
    virtual int EnableBluetoothToBleOnlyMode(std::string) = 0;
};
...
enum class BluetoothSwitchEvent : int {
    ...
    ENABLE_BLUETOOTH_TO_BLE_ONLY_MODE,
    BLUETOOTH_ON,
    BLUETOOTH_OFF,
    BLUETOOTH_HALF,
    BLUETOOTH_BLE_ONLY,
...
private:
    ...
    int ProcessEnableBluetoothToBleOnlyModeEvent(const std::string &callingName);
    int ProcessBluetoothOnEvent(void);
    int ProcessBluetoothOffEvent(void);
    int ProcessBluetoothHalfEvent(void);
    int ProcessBluetoothBleOnlyEvent(void);
