...
BluetoothRemoteDevice::BluetoothRemoteDevice(const int32_t &addressType, const std::string &addr, const int transport)
{
    address_ = addr;
...
int BluetoothRemoteDevice::StartPair()
{
    ...
    sptr<IBluetoothHost> hostProxy = GetRemoteProxy<IBluetoothHost>(BLUETOOTH_HOST);
    CHECK_AND_RETURN_LOG_RET(hostProxy != nullptr, BT_ERR_INTERNAL_ERROR, "proxy is nullptr.");
    BluetoothAddress bluetoothAddress(address_, addressType_);
    return hostProxy->StartPair(transport_, bluetoothAddress);
}
