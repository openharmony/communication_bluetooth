...
public:
    ...
    virtual int32_t SetRemoteDeviceBatteryInfo(const std::string &address, const BluetoothBatteryInfo &info) = 0;
    ...
    virtual int32_t GetRealAddress(const std::string &randomAddr, std::string &realAddr) = 0;
    ...
    virtual int32_t GetDeviceTransport(const std::string &address, int32_t &transport) = 0;
    virtual int32_t UpdateRefusePolicy(const int32_t protocolType,
        const int32_t pid, const int64_t prohibitedSecondsTime) = 0;
    ...
    virtual int32_t VerifyMultiPermissions(bool systemHapNeeded, const std::set<std::string> &permissionsApi9,
        const std::set<std::string> &permissionsApi10) = 0;
};
}  // namespace Bluetooth
}  // namespace OHOS
