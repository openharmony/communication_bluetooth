 ...
 */
#ifndef LOG_TAG
#define LOG_TAG "bt_ipc_parcel_gatt_character"
#endif
...
namespace OHOS {
namespace Bluetooth {
const uint32_t GATT_CHARACTERISTIC_PARCEL_SIZE_MAX = 0x400;
...
bool BluetoothGattCharacteristic::IsReadParcelDataSuccess(Parcel &parcel)
{
...
{
    if (!IsReadParcelDataSuccess(parcel)) {
        HILOGE("read parcel data error");
        return false;
    }
    uint32_t length = 0;
    ...
    uint32_t size;
    if (!parcel.ReadUint32(size) || size > GATT_CHARACTERISTIC_PARCEL_SIZE_MAX) {
