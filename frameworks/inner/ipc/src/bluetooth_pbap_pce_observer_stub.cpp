 ...
 */
#ifndef LOG_TAG
#define LOG_TAG "bt_ipc_pbap_pce_observer_stub"
#endif
...
BluetoothPbapPceObserverStub::BluetoothPbapPceObserverStub()
{
    memberFuncMap_[
        ...
        BluetoothPbapPceObserverStub::OnConnectionStateChangedInner;
    memberFuncMap_[
        static_cast<uint32_t>(
        BluetoothPbapPceObserverInterfaceCode::PBAP_PCE_ON_SYNC_STATE_CHANGE)] =
        BluetoothPbapPceObserverStub::OnSyncStateChangeInner;
}
...
int32_t BluetoothPbapPceObserverStub::OnRemoteRequest(
...
int32_t BluetoothPbapPceObserverStub::OnConnectionStateChangedInner(
    BluetoothPbapPceObserverStub *stub, MessageParcel &data, MessageParcel &reply)
{
    ...
    HILOGI("OnConnectionStateChangedInner, state: %{public}d, cause: %{public}d", state, cause);
    stub->OnConnectionStateChanged(*device, state, cause);
    return BT_NO_ERROR;
}

int32_t BluetoothPbapPceObserverStub::OnSyncStateChangeInner(
    BluetoothPbapPceObserverStub *stub, MessageParcel &data, MessageParcel &reply)
{
    std::shared_ptr<BluetoothRawAddress> device(data.ReadParcelable<BluetoothRawAddress>());
    CHECK_AND_RETURN_LOG_RET((device != nullptr), BT_ERR_INTERNAL_ERROR, "Read device error");
    int32_t syncState = data.ReadInt32();

    HILOGI("OnSyncStateChangedInner, syncState: %{public}d", syncState);
    stub->OnSyncStateChange(*device, syncState);
    return BT_NO_ERROR;
}
}  // namespace Bluetooth
}  // namespace OHOS