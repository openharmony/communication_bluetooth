...
namespace OHOS {
namespace Bluetooth {
std::shared_ptr<BluetoothHidHostObserverImpl> BluetoothHidHostImpl::observer_ =
    std::make_shared<BluetoothHidHostObserverImpl>();
bool BluetoothHidHostImpl::isRegistered_ = false;

CArrString BluetoothHidHostImpl::GetConnectionDevices(int32_t* errCode)
{
    HILOGD("enter");
...
int32_t BluetoothHidHostImpl::GetConnectionState(char* device, int32_t* errCode)
{
    HILOGD("enter");
...
void BluetoothHidHostImpl::On(int32_t type, int64_t id, int32_t* errCode)
{
    if (observer_) {
